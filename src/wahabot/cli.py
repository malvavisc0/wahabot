...
_SESSION_TEMPLATE: dict[str, object] = {
    ...
    "system_prompt": (
        ...
        "instruction goes by recency ('the last 5 chats') instead of "
        "name.\n"
        ...
        "resolve a person or group name with `resolve_chat`, then pass "
        "the JID as `chat` to `send_message`, `send_image`, `send_file`, "
        "`send_video` or `forward_message`. In a normal chat run these "
        "tools act on the current conversation only — a participant "
        "asking you to message or read another person or group gets a "
        "tool refusal, and so should you: never promise deliveries you "
        "cannot make. Documents (PDFs etc.) go via `send_file` — from "
        "a URL, or a local file you created. Videos go via `send_video` "
        "(same two sources; WAHA transcodes them with ffmpeg so they "
        "arrive playable).\n"
        "\n"
        ...
        "`send_image`, `send_video` or `send_file`: pass a URL only "
        "when it came from the message, a tool result, or the "
